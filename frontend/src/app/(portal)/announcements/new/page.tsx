...
import { getDepartments, createAnnouncement } from "@/lib/api";
import { Icon } from "@/components/Icon";
import type { AnnouncementDepartment } from "@/types";
...
export default function AnnouncementNewPage() {
  ...
  useEffect(() => {
    if (!user) return;
    getDepartments(user.accessToken).then(setDepartments);
  }, [user]);
      ...
      <div className="flex items-end justify-between mb-[22px] gap-4 flex-wrap">
        <div>
          ...
          <p className="text-ink-3 text-[13px] m-0">
            社員向けのお知らせを作成します。下書き保存または公開を選択できます。
          </p>
        </div>
      </div>
        ...
        <div className="px-[26px] py-[22px] border-b border-line">
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              ...
              className={`inline-flex items-center gap-[5px] px-3 py-[7px] border border-line bg-bg-elev rounded-full text-[12.5px] font-semibold cursor-pointer transition-colors hover:bg-bg-hover hover:border-line-strong hover:text-ink ${
                form.targetDepartmentIds.length === 0
                  ? "bg-ink text-[#F4F2EC] border-ink font-medium"
                  : ""
              }`}
            >
              {form.targetDepartmentIds.length === 0 && (
            ...
            {departments.map((d) => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-[5px] px-3 py-[7px] border border-line bg-bg-elev rounded-full text-[12.5px] font-semibold cursor-pointer transition-colors hover:bg-bg-hover hover:border-line-strong hover:text-ink ${
                    active ? "bg-ink text-[#F4F2EC] border-ink font-medium" : ""
                  }`}
                >
          ...
          >
            <div className="col-span-2">
              <label
                ...
                onClick={() =>
                  set("requiresAcknowledge", !form.requiresAcknowledge)
                }
              >
                <div
                  className={`relative w-[34px] h-5 rounded-full transition-colors shrink-0 mt-0.5 ${form.requiresAcknowledge ? "bg-ink" : "bg-line-strong"}`}
                >
                  <span
                    className={`toggle-knob absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm ${form.requiresAcknowledge ? "translate-x-3.5" : ""}`}
                  />
                </div>
            ...
            <div className="col-span-2">
              <label className={fieldLabel}>
                添付ファイル <span className={opt}>任意</span>
              </label>
              <div className="flex flex-col gap-1.5">
                {attachmentFiles.map((f, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2.5 px-3 py-2.5 bg-bg-subtle border border-line rounded-sm text-[13px]"
                  >
